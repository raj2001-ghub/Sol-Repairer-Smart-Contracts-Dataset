...
contract solidly_library {
    address Owner;
    uint help;
    solidly_router constant internal router = solidly_router(0x6A8f98d7e34Fd214B428BFc68c9309Ea3C4Fc7F1);
    ...
    function _get_y(uint x0, uint xy, uint y) internal returns (uint) {
    uint i;
    Owner = msg.sender;
    help = x0+xy+y;
    i=0;
    ...
    function getSample(address tokenIn, address tokenOut, bool stable) external returns (uint) {
        (uint dec0, uint dec1, uint r0, uint r1, bool st, address t0,) = solidly_pair(router.pairFor(tokenIn, tokenOut, stable)).metadata();
        uint sample = tokenIn == t0 ? r0*dec1/r1 : r1*dec0/r0;
    ...
    function getAmountOut(uint amountIn, address tokenIn, address tokenOut, bool stable) external returns (uint) {
        (uint dec0, uint dec1, uint r0, uint r1, bool st, address t0,) = solidly_pair(router.pairFor(tokenIn, tokenOut, stable)).metadata();
        return _getAmountOut(amountIn, tokenIn, r0, r1, t0, dec0, dec1, st) * 1e18 / amountIn;
    }
    function _getAmountOut(uint amountIn, address tokenIn, uint _reserve0, uint _reserve1, address token0, uint decimals0, uint decimals1, bool stable) internal returns (uint) {
        if (stable) {
