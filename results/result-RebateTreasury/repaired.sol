...
//*        totalVested += zombieAmount;
    ...
    function redeemAssetsForBuyback(address[] calldata tokens) external onlyOwner {
        require(getZombiePrice() < 1e17, "RebateTreasury: unable to buy back");
    ...
    function getBondPremium() public view returns (uint256) {
        uint256 zombiePrice = getZombiePrice();
        if (zombiePrice < 1e17) return 0; // related to 0.1AVAX peg
        uint256 zombiePremium = zombiePrice * DENOMINATOR / 1e17 - DENOMINATOR; // related to 1AVAX peg
