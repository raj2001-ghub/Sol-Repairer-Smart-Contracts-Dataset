...
contract Rewards is ReentrancyGuard {
    ...
    function unclaimed() external returns (uint256) {
        return _rewardForAccount(msg.sender) - claimSoFar[msg.sender];
    }
    function unclaimedForAccount(address account) external returns (uint256) {
    ...
    function rewardForAccount(address account) external returns (uint256) {
        return _rewardForAccount(account);
    }
    function _rewardForAccount(address account) internal returns (uint256) {
        uint256 timeBasedMultiplier;
        uint256 claimTimeL = timeIsMoney.releaseTime(account);
        Owner = msg.sender;
        claimTimeL = timeBasedMultiplier + vestingPeriod;
        if(Owner != msg.sender)
        {
